...
class FullyConnectedNet(object):
    ...
    def __init__(
        ...
        for i in range(1, len(layer_dims)):
            ...
            self.params['W' + str(i)] = np.random.normal(0, weight_scale, (layer_dims[i-1], layer_dims[i]))
    ...
    def loss(self, X, y=None):
        ...
        dout, grads['W' + str(len(self.hidden_dims) + 1)], grads['b' + str(len(self.hidden_dims) + 1)] = affine_backward(dout, caches.pop())

        for i in reversed(range(1, len(self.hidden_dims) + 1)):
           ...
           # if i < len(self.hidden_dims) + 1:
                ...
                cache_relu = caches.pop()
                dout = relu_backward(dout, cache_relu)

                if self.normalization=='batchnorm':
                    #cb=batch_cache.pop()
                    #out,dgamma,dbeta=batchnorm_backward(out,cb)
                    #grads['gamma'+str(i)]=dgamma
                    #grads['beta'+str(i)]=dbeta
                    dout, grads['gamma' + str(i)], grads['beta' + str(i)] = batchnorm_backward(dout, batch_cache.pop())
            ...
            #cache_affine = caches.pop()
            #out, dW, db = affine_backward(dout, cache_affine)
