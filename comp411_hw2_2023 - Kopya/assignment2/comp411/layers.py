...
def affine_forward(x, w, b):
    ...
    N=x.shape[0]
    input_reshaped=x.reshape(N,-1)
    out=input_reshaped.dot(w)+b
...
def batchnorm_backward(dout, cache):
    ...
    # *****START OF YOUR CODE (DO NOT DELETE/MODIFY THIS LINE)*****
    N, D = dout.shape
    x_normalized, mean, variance, gamma, beta, eps, x = cache['x_normalized'], cache['mean'], cache['variance'], cache['gamma'], cache['beta'], cache['eps'], cache['x']
    
    dbeta = np.sum(dout, axis=0)
    dgamma = np.sum(dout * x_normalized, axis=0)
    dx_normalized = dout * gamma
    dvariance = np.sum(dx_normalized * (x - mean) * (-0.5) * (variance + eps)**(-1.5), axis=0)
    dmean = np.sum(dx_normalized * (-1 / np.sqrt(variance + eps)), axis=0) + dvariance * np.mean(-2 * (x - mean), axis=0)
...
def batchnorm_backward_alt(dout, cache):
    ...
    # *****START OF YOUR CODE (DO NOT DELETE/MODIFY THIS LINE)*****
    N, D = dout.shape
    x_normalized, gamma, mean, variance, eps = cache['x_normalized'], cache['gamma'], cache['mean'], cache['variance'], cache['eps']
   

    dbeta = np.sum(dout, axis=0)
    dgamma = np.sum(dout * x_normalized, axis=0)
    dx_normalized = dout * gamma
    dx = (1. / N) * (1. / np.sqrt(variance + eps)) * (N * dx_normalized - np.sum(dx_normalized, axis=0)
...
def conv_forward_naive(x, w, b, conv_param):
    ...
    padded_input=np.pad(x,((0,0),(0,0),(pad,pad),(pad,pad)),mode='constant')
        ...
        for j in range(F):
            for k in range(Hout):
                for m in range(Wout):
                    ...
                    w0=m*stride
                    wf=w0+WW
                    receptive=padded_input[i,:,h0:hf,w0:wf]
                    out[i,j,k,m]=np.sum(receptive*w[j])+b[j]
        ...
        for j in range(F):
            for k in range(Hout):
                for m in range(Wout):
                    ...
                    dw[j] += x_pad[i, :,h0:hf, w0:wf] * dout[i, j, k, m]
                    dx_pad[i, :, h0:hf, w0:wf] += w[j]*dout[i,j,k,m]
                    db[j] += dout[i, j, k, m]
        ...
        for j in range(C):
            for k in range(Hout):
                for m in range(Wout):
                    ...
                    w0=m*stride
                    wf=w0+width
                    window=x[i,j,h0:hf,w0:wf]
                    out[i,j,k,m]=np.max(window)
        ...
        for j in range(C):
            for k in range(Hout):
                for m in range(Wout):
                    ...
                    window=x[i,j,h0:hf,w0:wf]
                    max=np.max(window)
                    mask=(window==max) #we are making the indices of max in the window 1 and others 0
                    dx[i,j,h0:hf,w0:wf]+=mask*dout[i,j,k,m]
        ...
        for j in range(C):
            for k in range(Hout):
                for m in range(Wout):
                    ...
                    wf=w0+width
                    window=x[i,j,h0:hf,w0:wf]
                    base=np.ones_like(window)
                    dx[i,j,h0:hf,w0:wf]+=base*(dout[i,j,k,m]/(height*width))
