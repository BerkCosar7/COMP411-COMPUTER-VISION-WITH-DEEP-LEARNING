...
class CaptioningRNN(object):
    ...
    def loss(self, features, captions):
        ...
        words,cache_embedded=word_embedding_forward(captions_in,W_embed)

        if self.cell_type=="rnn":
            h,cache_rnn=rnn_forward(words,h0,Wx,Wh,b)

        scores,cache_scores=temporal_affine_forward(h,W_vocab,b_vocab)
        loss,dscores=temporal_softmax_loss(scores,captions_out,mask,verbose=0)
        dh,dW_vocab,db_vocab=temporal_affine_backward(dscores,cache_scores)

        if self.cell_type=="rnn":
            dwords,dh0,dWx,dWh,db=rnn_backward(dh,cache_rnn)

        dW_embedded=word_embedding_backward(dwords,cache_embedded)
    ...
    def sample(self, features, max_length=30):
        ...
        for t in range(max_length):
            words,_=word_embedding_forward(prev_word,W_embed)
            next_h,_=rnn_step_forward(words,h0,Wx,Wh,b)
